...
export class UART extends EventEmitter {
    ...
    write(data) {
        return new Promise((resolve, reject) => {
            if (!Buffer.isBuffer(data)) {
                reject(new TypeError('Expected Buffer'));
                return;
            }
            trace('Writing:', data);
    ...
    send(command = '') {
        ...
        return this.write(Buffer.concat([Buffer.from(command), delimiterBuffer])).then(() => {
    ...
    unbind() {
        ...
        if (_.isFunction(port.close)) {
            return pify(port.close.bind(port))().then(() => {
                this.emit('disconnect');
                return this;
            });
        }
    ...
    static from(port, opts = {}) {
        return isValidPort(port) ? UART.fromSerial(port, opts) : UART.fromPort(port, opts);
    }
     ...
     */
    static findPort() {
        return pify(Serialport.list)().then(
            _.pipe(
                _.pluck('comName'),
                _.filter((portName) => PORT_GUESS_REGEX.test(portName)),
                _.head,
            ),
        );
    }
