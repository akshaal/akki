 ...
 * @private
 */
export const SYSTEM_VARIABLES = ['sys0', 'sys1', 'sys2'];
    ...
    setSystemVariable(name, value) {
        return Promise.resolve().then(() => {
            ...
            if (!isUnsignedInteger(value)) {
                throw new TypeError(`"value" must be an unsigned integer <= ${MAX_INT}; got ${value}`);
            }
    ...
    setRandomRange(min = 0, max = 1) {
        return Promise.resolve().then(() => {
            if (!isUnsignedInteger(min)) {
                throw new TypeError(`"min" must be an unsigned integer <= ${MAX_INT}`);
            }
            if (!isUnsignedInteger(max)) {
                throw new TypeError(`"max" must be an unsigned integer <= ${MAX_INT}`);
            }
            if (min > max) {
